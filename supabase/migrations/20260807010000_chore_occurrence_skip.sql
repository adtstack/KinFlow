-- KinFlow WP03-05B single-occurrence skip.
-- Store MVP scope: online-only, versioned skip for one repeating occurrence.

alter table public.chore_completion_events
  drop constraint chore_completion_events_event_type_check;

alter table public.chore_completion_events
  add constraint chore_completion_events_event_type_check
  check (event_type in ('completed', 'reopened', 'skipped'))
  not valid;

alter table public.chore_completion_events
  validate constraint chore_completion_events_event_type_check;

create table app_private.chore_skip_command_requests (
...
begin
  ...
  from app_private.chore_skip_command_requests as request
  ...
  insert into app_private.chore_skip_command_requests (
...
revoke all on function public.skip_chore_occurrence(
  uuid,
  uuid,
  uuid,
  bigint
...
grant execute on function public.skip_chore_occurrence(
  uuid,
  uuid,
  uuid,
  bigint
