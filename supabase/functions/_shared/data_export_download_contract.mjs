export const dataExportDownloadContractVersion = "2026-08-08-wp07-02a";

const maximumArtifactBytes = 10 * 1024 * 1024;
const tokenPattern = /^[A-Za-z0-9_-]{43}$/;
const objectKeyPattern = /^exports\/[0-9a-f-]{36}\/kinflow-data\.(json|txt)$/;
const uuidPattern = /^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i;

export class DataExportDownloadFailure extends Error {
  constructor(code = "TEMPORARILY_UNAVAILABLE") {
    super("Data export download failed");
    this.name = "DataExportDownloadFailure";
    this.code = code;
  }
}

export function createDataExportDownloadHandler({consumeGrant, downloadObject}) {
  return async function handleDataExportDownload(request) {
    ...
    try {
      ...
      if (!(bytes instanceof Uint8Array) || bytes.byteLength !== artifact.size_bytes ||
        ...
        throw new DataExportDownloadFailure();
      }
      ...
      const invalid = error instanceof DataExportDownloadFailure &&
...
function artifactRow(payload) {
  if (!Array.isArray(payload) || payload.length !== 1 || !isPlainObject(payload[0])) {
    throw new TypeError("Invalid download grant payload");
  }
  const row = payload[0];
  exactKeys(row, [
    "checksum_sha256",
  ...
  const format = row.export_format;
  const expected = format === "json"
    ? ["kinflow-data.json", "application/json; charset=utf-8", "json"]
    : ["kinflow-data.txt", "text/plain; charset=utf-8", "txt"];
  if (!["json", "text"].includes(format) ||
    ...
    throw new TypeError("Invalid download artifact metadata");
  }
  return row;
}
...
function exactKeys(value, expected) {
  ...
  if (actual.length !== sorted.length || actual.some((key, index) => key !== sorted[index])) {
    throw new TypeError("Unexpected download artifact fields");
  }
}
