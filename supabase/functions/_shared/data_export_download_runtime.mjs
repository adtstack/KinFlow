...
} from "./data_export_download_contract.mjs";

export function serveDataExportDownload() {
  const supabaseUrl = requiredEnvironment("SUPABASE_URL").replace(/\/$/, "");
  const serviceRoleKey = requiredEnvironment("SUPABASE_SERVICE_ROLE_KEY");
  Deno.serve(createDataExportDownloadHandler({
    consumeGrant: (parameters) => invokeRpc({parameters, serviceRoleKey, supabaseUrl}),
    downloadObject: (objectKey) => downloadObject({objectKey, serviceRoleKey, supabaseUrl}),
  }));
}
...
async function invokeRpc({parameters, serviceRoleKey, supabaseUrl}) {
  let response;
  try {
    response = await fetch(`${supabaseUrl}/rest/v1/rpc/consume_data_export_download_grant`, {
      method: "POST",
      headers: {
  ...
  if (!response.ok) {
    const invalid = ["KFX10", "KFX12"].includes(payload?.code);
    throw new DataExportDownloadFailure(
...
async function downloadObject({objectKey, serviceRoleKey, supabaseUrl}) {
  ...
  if (!response.ok) throw new DataExportDownloadFailure();
