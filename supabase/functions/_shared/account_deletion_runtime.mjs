...
} from "./account_deletion_contract.mjs";

export function serveAccountDeletion() {
...
async function authenticate({apiKey, authorization, supabaseUrl}) {
  ...
  try {
    ...
    throw new AccountDeletionRpcError("AUTH_PROVIDER_UNAVAILABLE");
  }
...
async function invokeRpc({name, parameters, serviceRoleKey, supabaseUrl}) {
  ...
  try {
    ...
    throw new AccountDeletionRpcError("RPC_UNAVAILABLE");
  }
  const payload = await safeJson(response);
  if (!response.ok) throw new AccountDeletionRpcError(payload?.code);
  return payload;
}
