import {createDataExportHandler, DataExportRpcError} from "./data_export_contract.mjs";

export function serveDataExport() {
  ...
  const allowedOrigins = (environment("KINFLOW_ALLOWED_ORIGINS") || "http://127.0.0.1:3000")
    ...
    .filter(Boolean);
  const downloadBaseUrl = environment("KINFLOW_DATA_EXPORT_DOWNLOAD_URL") ||
    `${supabaseUrl}/functions/v1/data-export-download`;
  Deno.serve(createDataExportHandler({
    allowedOrigins,
    downloadBaseUrl,
    authenticate: (authorization) => authenticate({apiKey, authorization, supabaseUrl}),
    invokeRpc: (name, parameters) => invokeRpc({name, parameters, serviceRoleKey, supabaseUrl}),
  }));
}
...
async function authenticate({apiKey, authorization, supabaseUrl}) {
  ...
  try {
    ...
    throw new DataExportRpcError("AUTH_PROVIDER_UNAVAILABLE");
  }
...
async function invokeRpc({name, parameters, serviceRoleKey, supabaseUrl}) {
  ...
  try {
    ...
    throw new DataExportRpcError("RPC_UNAVAILABLE");
  }
  const payload = await safeJson(response);
  if (!response.ok) throw new DataExportRpcError(payload?.code);
  return payload;
}
